...
module vslot(length=50, sections=1, finish, cutout=0) {
	size = sections * 20;
	...
	module profile() {
      ...
      if (cutout == 1){
		 translate([0,0,length/2]) box((20 * sections)+.05, 20.05, length);
          } else {
	     if(finish)
...
module vslot20x20_cutout(length, finish) { vslot(length, 1, finish, 1); }


module vslot20x40(length, finish) { vslot(length, 2, finish); }
module vslot20x40_cutout(length, finish) { vslot(length, 2, finish, 1); }

module vslot20x60(length, finish) { vslot(length, 3, finish); }
module vslot20x60_cutout(length, finish) { vslot(length, 3, finish, 1); }

module vslot20x80(length, finish) { vslot(length, 4, finish); }
module vslot20x80_cutout(length, finish) { vslot(length, 4, finish, 1); }
